...
class FootingDesignService {
  ...
  /// METHOD: runPipeline
  /// Bridges the UI state with the engineering logic.
  FootingDesignState runPipeline(FootingDesignState state) {
    // 1. Convert state to domain input
    final input = FootingInput(
      ...
      foundationDepth: state.foundationDepth,
      concreteGrade: state.concreteGrade,
      steelGrade: state.steelGrade,
    );

    // 2. Perform engineering calculation
    final result = designFooting(input);
    ...
    return state.copyWith(
      ...
      astRequiredX: result.astRequiredX,
      astRequiredY: result.astRequiredY,
      astProvidedX: result.astProvidedX,
      astProvidedY: result.astProvidedY,
      mainBarSpacing: result.mainBarSpacing,
      crossBarSpacing: result.crossBarSpacing,
  ...
  FootingResult designFooting(FootingInput input) {
    ...
    // 4. Reinforcement
    final d = input.footingThickness - 50; // 50mm cover
