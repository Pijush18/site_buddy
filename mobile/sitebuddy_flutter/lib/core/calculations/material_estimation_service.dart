...
class MaterialEstimationService {
  ...
  /// Estimates the excavation volume for a pit or trench.
  /// Includes clearance and swell factor (bulking).
  ExcavationResult calculateExcavationVolume({
    required double length,
    required double width,
    required double depth,
    double clearance = 0.3, // 300mm standard clearance
    double swellFactor = 1.25, // Bank to Loose volume
  }) {
    _assertPositive(length, 'length');
    _assertPositive(width, 'width');
    _assertPositive(depth, 'depth');

    final effectiveLength = length + (2 * clearance);
    final effectiveWidth = width + (2 * clearance);
    final bankVolume = effectiveLength * effectiveWidth * depth;
  ...
  /// Estimates the shuttering (formwork) area for concrete elements.
  ShutteringResult calculateShutteringArea({
    required double length,
    required double width,
    required double depth,
    ...
    final sideArea = 2 * (length + width) * depth;
    final bottomArea = includeBottom ? (length * width) : 0.0;
